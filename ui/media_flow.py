...
class MediaFlowWidget(QFrame):
    def __init__(self, index: int, relative_path: str, torrent_bytes: bytes, image_url: str, title: str, parent=None):
        ...
        self.setStyleSheet("""
            QFrame { background-color: #ffffff; border-bottom: 1px solid #e2e8f0; padding: 5px; }
            QLabel { color: #334155; font-size: 11pt; font-family: 'Segoe UI'; }
            QPushButton { background-color: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; border-radius: 12px; padding: 5px 15px; font-weight: bold; font-size: 9pt; }
            QPushButton:hover { background-color: #e2e8f0; }
        """)
        self.setFixedHeight(60)

        self.main_layout = QHBoxLayout(self)
        self.main_layout.setContentsMargins(10, 5, 20, 5)

        self.title_lbl = QLabel(f"{self.flow_index}. {self.title}")
        self.title_lbl.setStyleSheet("font-size: 12pt;")
        self.main_layout.addWidget(self.title_lbl, stretch=2)

        self.btn_download = QPushButton(" Downloading")
        self.btn_convert = QPushButton(" Pending")
        self.btn_finish = QPushButton(" Incomplete")
        ...
        arrow1 = QLabel("➔")
        arrow1.setStyleSheet("color: #94a3b8; font-size: 10pt;")
        ...
        arrow2 = QLabel("➔")
        arrow2.setStyleSheet("color: #94a3b8; font-size: 10pt;")
    ...
    def _start_flow(self) -> None:
        self.btn_download.setStyleSheet("QPushButton { border: 2px solid #3b82f6; background-color: #eff6ff; color: #2563eb; }")
        ...
        try:
            ...
            c = qbittorrentapi.Client(host=f"{h}:{p}", username=os.getenv("QBIT_USER", "admin"), password=os.getenv("QBIT_PASS", "adminadmin"))
            c.auth_log_in()
    ...
    def _update_torrent_ui(self, data_payload: List[Any]) -> None:
        ...
        if target_torrents:
            ...
            if prog_val < 1.0 and state not in ['error', 'missingFiles']:
                ...
                self.btn_download.setStyleSheet("QPushButton { border: 2px solid #f59e0b; background-color: #fffbeb; color: #d97706; }")
            elif state in ['error', 'missingFiles']:
                self.btn_download.setText(" Torrent Error")
                self.btn_download.setStyleSheet("QPushButton { border: 2px solid #ef4444; background-color: #fef2f2; color: #dc2626; }")
            ...
            if prog_val == 1.0 or state in ['uploading', 'stalledUP', 'pausedUP', 'completed']:
                 if not hasattr(self, 'ssh_timer'):
                     ...
                     self.btn_download.setStyleSheet("QPushButton { background-color: #f1f5f9; color: #64748b; border: 1px solid #cbd5e1; }")
                     
                     self.btn_convert.setText(" Encoding 0%")
                     self.btn_convert.setStyleSheet("QPushButton { border: 2px solid #f59e0b; background-color: #fffbeb; color: #d97706; }")
    ...
    def _pull_ssh(self) -> None:
        if hasattr(self, 'ssh_worker') and self.ssh_worker.isRunning(): return
    ...
    def _update_telemetry_ui(self, db_out: str, gen_out: str, ff_out: str, prog: int) -> None:
        ...
        if prog >= 100:
            ...
            self.btn_convert.setStyleSheet("QPushButton { background-color: #f1f5f9; color: #64748b; border: 1px solid #cbd5e1; }")
            
            self.btn_finish.setText(" Approve & Share")
            self.btn_finish.setStyleSheet("QPushButton { border: 2px solid #10b981; background-color: #ecfdf5; color: #059669; }")
